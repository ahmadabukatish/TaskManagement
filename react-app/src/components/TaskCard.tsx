...
function TaskCard({index}:Props)
{
    ...
    {
        Axios.delete(`http://localhost:5180/api/delete/${item.title}`).catch(e=>{navigate('../errorPage')});
        let temp=[...itemsArr];
        temp.splice(index,1)
    ...
    {
        Axios.put("http://localhost:5180/api/update",{title:item.title,status:e.target.value}).catch(e=>{navigate('../errorPage')});
        let temp=[...itemsArr];
        temp.splice(index,1)
